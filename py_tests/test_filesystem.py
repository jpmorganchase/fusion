...
def make_response(resources: list[Any], next_token: Optional[str] = None) -> MagicMock:
    resp = MagicMock()
...
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
def test_ls_multiple_pages() -> None:
    fs = FusionHTTPFileSystem()
    fs.sync_session = MagicMock()
    with (
        patch.object(fs, "_decorate_url", MagicMock(side_effect=lambda x: x)),
...
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
async def test__ls_single_page() -> None:
    fs = FusionHTTPFileSystem()
    fs.client_kwargs = {"root_url": "https://fusion.jpmorgan.com/api/v1/"}
    fs.kwargs = {}
    with patch.object(fs, "_decorate_url_a", AsyncMock(side_effect=lambda url: url)):
        resources = [
            {
                "name": "https://fusion.jpmorgan.com/api/v1/catalogs/foo",
                "identifier": "https://fusion.jpmorgan.com/api/v1/catalogs/foo",
...
@patch.object(FusionHTTPFileSystem, "_async_startup", AsyncMock())
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
async def test__ls_multiple_pages() -> None:
    fs = FusionHTTPFileSystem()
    fs.client_kwargs = {"root_url": "https://fusion.jpmorgan.com/api/v1/"}
    fs.kwargs = {}
    with patch.object(fs, "_decorate_url_a", AsyncMock(side_effect=lambda url: url)):
...
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
def test_cat_single_page() -> None:
    fs = FusionHTTPFileSystem()
    fs.sync_session = MagicMock()
    with (
        patch.object(fs, "_decorate_url", MagicMock(side_effect=lambda url: url)),
        patch.object(fs, "sync_session", MagicMock()),
    ):
        fs.kwargs = {}
...
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
def test_cat_multiple_pages() -> None:
    fs = FusionHTTPFileSystem()
    fs.sync_session = MagicMock()
    with (
        patch.object(fs, "_decorate_url", MagicMock(side_effect=lambda url: url)),
...
@pytest.mark.asyncio
@patch.object(FusionHTTPFileSystem, "_async_startup", AsyncMock())
@patch.object(FusionHTTPFileSystem, "_decorate_url", lambda _, url: url)
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
async def test__cat_single_page() -> None:
    fs = FusionHTTPFileSystem()
    fs.kwargs = {}
    mock_response = AsyncMock()
...
@patch.object(FusionHTTPFileSystem, "_decorate_url", lambda _, url: url)
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
async def test__cat_multiple_pages() -> None:
    fs = FusionHTTPFileSystem()
    fs.kwargs = {}
    resp1 = AsyncMock()
    ...
    result = await fs._cat("some_url")
    assert result == b"abcdef"
    assert mock_session.get.call_count == PAGINATED
...
@patch.object(FusionHTTPFileSystem, "_decorate_url", lambda _, url: url)
@patch.object(FusionHTTPFileSystem, "__init__", lambda *_, **__: None)
async def test__cat_with_range() -> None:
    fs = FusionHTTPFileSystem()
    fs.kwargs = {}
