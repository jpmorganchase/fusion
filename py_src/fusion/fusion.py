...
class Fusion:
    ...
    def __init__(
        ...
        if isinstance(credentials, FusionCredentials):
            ...
            raise ValueError(
                "credentials must be a path to a credentials file or FusionCredentials object"
            )
        ...
        for entry in relations_data:
            ...
            if destination_dataset_id == dataset_id:
                for dataset in data["datasets"]:
                    if (
                        dataset["identifier"] == source_dataset_id
                        and dataset.get("status", None) != "Restricted"
                    ):
            ...
            if source_dataset_id == dataset_id:
                for dataset in data["datasets"]:
                    if (
                        dataset["identifier"] == destination_dataset_id
                        and dataset.get("status", None) != "Restricted"
                    ):
    ...
    def create_dataset_lineage(
        ...
        if isinstance(source_dataset_catalog_mapping, pd.DataFrame):
            ...
            raise ValueError(
                "source_dataset_catalog_mapping must be a pandas DataFrame or a list of dictionaries."
            )
        ...
        if dataset:
            if isinstance(dataset, list):
                ...
                mapping_df = mapping_df[
                    mapping_df["dataset"].str.contains(contains, case=False)
                ]
            if isinstance(dataset, str):
                mapping_df = mapping_df[
                    mapping_df["dataset"].str.contains(dataset, case=False)
                ]
        if product:
            if isinstance(product, list):
                contains = "|".join(f"{s}" for s in product)
                mapping_df = mapping_df[
                    mapping_df["product"].str.contains(contains, case=False)
                ]
            if isinstance(product, str):
                mapping_df = mapping_df[
                    mapping_df["product"].str.contains(product, case=False)
                ]
        return mapping_df
